...
void  fvimatchl32_transpose_kernel(int ndim,   type *A, type *B,  const int *lda, const int *ldb, const int* params, const int * rperm, type alpha, type beta) 
{
	...
	SAFECUDAMEMCPY(d_offset, offset,limit*sizeof(short), hipMemcpyHostToDevice); 
	...
	SAFECUDAMEMCPY(d_idx_s, idx_s+1,newndim*sizeof(int), hipMemcpyHostToDevice); 
	SAFECUDAMEMCPY(d_lda_s, lda_s+1,newndim*sizeof(int), hipMemcpyHostToDevice); 
	SAFECUDAMEMCPY(d_ldb_s, temp+1,newndim*sizeof(int), hipMemcpyHostToDevice); 
