...

{
	...
	switch(ndim)
	{
		...
		default: {}
			 //                       hipLaunchKernelGGL((fvinomgeneralolap_kernel), dim3(thread_blocks), dim3(numthreads), shm, 0, ndim, A,  B, ilimit, olimit, i_blkindex, b_blkindex, numthreads, lda_s,ldb_s, idx_s, remainder1,remainder2, aexpr, bexpr, texpr1, texpr2, ilimitr, olimitr, inputrem, outputrem);
	}
...
printf("\t%d\t%d\t", ilimit, olimit);
	...
	SAFECUDAMEMCPY(input_base, aexpr, olimit*sizeof(int), hipMemcpyHostToDevice); 
	SAFECUDAMEMCPY(output_base, bexpr, ilimit*sizeof(int), hipMemcpyHostToDevice); 
