...

{
	...
	if(size > 0)
	{
		...
		switch(ndim)
		{
			...
			default:
			{
				//			hipLaunchKernelGGL((fvinomgeneralolap_coars_kernel), dim3(thread_blocks), dim3(numthreads), shm, 0, ndim, A,  B, ilimit, olimit, blockAI, blockBI, numthreads, lda_s,ldb_s, idx_s, coarsa, coarsb, aexpr, bexpr, texpr1, texpr2, ilimitr, olimitr, inputrem, outputrem, rowinc, shm2, numelems_blk);
			}
		}
	}
	else
	{
		...
		switch(ndim)
		{
			...
			default:
			{
				//			hipLaunchKernelGGL((fvinomgeneralolap_kernel), dim3(thread_blocks), dim3(numthreads), shm, 0, ndim, A,  B, ilimit, olimit, blockAI, blockBI, numthreads, lda_s,ldb_s, idx_s, aexpr, bexpr, texpr1, texpr2, ilimitr, olimitr, inputrem, outputrem, rowinc, shm2, numelems_blk);
			}
		}
	}
